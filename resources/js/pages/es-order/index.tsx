...
function OrderIndexContent({ view }: OrderListPageProps) {
  ...
  const handleExport = () => {
    ...
    window.location.href = `/es-order/export?${params.toString()}`;
  };
      ...
      <Page.Content>
        {orders && orders.length === 0 ? (
          ...
          <div className="space-y-6">
            ...
            {/* Orders Data Table */}
            {pagination && metadata && <InertiaDataTable data={orders || []} pagination={pagination} metadata={metadata} rowClickRoute="/es-order/:id" />}
          </div>
        )}
      </Page.Content>
    </>
