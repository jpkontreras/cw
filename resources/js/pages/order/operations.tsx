    ...
    <Card className="cursor-pointer transition-shadow hover:shadow-lg">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="border-t pt-3">
          ...
          <Button className="w-full" size="sm" onClick={() => onAction?.(String(order.id), 'next')}>
  ...
  if (!hasOrders) {
    return (
      <EmptyState
        ...
        description="No orders in the kitchen queue. New orders will appear here when confirmed."
        helpText="Create an order or wait for customers to place orders to see them here."
      />
    );
  }
                ...
                <CardContent className="p-6">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-lg font-bold">
                      {order.tableNumber ? `Table ${order.tableNumber}` : formatOrderNumber(order.orderNumber)}
                    </span>
                    ...
                    {order.items.map((item: any, idx: any) => (
                      <div key={idx} className="text-sm">
                        ...
                        {item.modifiers && item.modifiers.length > 0 && (
                          <div className="ml-4 text-xs text-gray-600">{item.modifiers.map((m: any) => m.modifierName || m.modifier_name).join(', ')}</div>
                        )}
                  ...
                  <div className="space-y-2">
                    {order.items.map((item: any, idx: any) => (
                      <div key={idx} className="flex items-center justify-between text-sm">
                        ...
                        <Badge variant="outline" className={getKitchenStatusColor(item.kitchenStatus || item.kitchen_status)}>
                          {getKitchenStatusLabel(item.kitchenStatus || item.kitchen_status)}
                        </Badge>
                      </div>
                    ))}
                  </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-1">
                    <div className="text-sm text-gray-600">
                      {order.type === 'dineIn' && order.waiterId && <span>Waiter ID: {order.waiterId}</span>}
                      {order.type === 'delivery' && <span>Delivery to: {order.deliveryAddress}</span>}
                      {order.type === 'takeout' && <span>Customer: {order.customerName}</span>}
                    </div>
...
// Empty Operations View Component
const EmptyOperationsView = ({ fullscreen }: { fullscreen: boolean }) => (
  <div className={`${fullscreen ? 'p-6' : 'px-4'}`}>
    <EmptyState
      icon={Monitor}
      title="No active orders"
      description="Your operations center is ready. New orders will appear here automatically when they're placed."
      helpText="You can create an order manually using the button above, or wait for customers to place orders."
    />
  </div>
);

// Active Operations View Component
const ActiveOperationsView = ({
  ...
  fullscreen: boolean;
  stats: OperationsPageProps['stats'];
  selectedView: string;
  setSelectedView: (view: string) => void;
  viewMode: 'grid' | 'list';
  activeOrders: Order[];
  handleOrderAction: (orderId: string, action: string) => void;
}) => (
  <>
    {/* Stats Bar */}
    ...
    <Tabs value={selectedView} onValueChange={setSelectedView} className={fullscreen ? 'px-6' : 'px-4'}>
      ...
      <TabsContent value="delivery" className="mt-6">
        <DeliveryTrackerTab activeOrders={activeOrders} handleOrderAction={handleOrderAction} />
      </TabsContent>

      {/* Alerts Tab */}
      <TabsContent value="alerts" className="mt-6">
        <AlertsTab activeOrders={activeOrders} handleOrderAction={handleOrderAction} />
      </TabsContent>
    </Tabs>
  </>
);

// Delivery Tracker Tab Component
const DeliveryTrackerTab = ({ 
  activeOrders, 
  handleOrderAction 
}: { 
  activeOrders: Order[]; 
  ...
  if (deliveryOrders.length === 0) {
    return (
      <EmptyState
        ...
        description="Track and manage delivery orders from this dashboard when they're placed."
        helpText="Create a delivery order using the button above or wait for customer orders."
      />
    );
  }
        ...
        <CardContent>
          <div className="space-y-4">
            {deliveryOrders.filter((o) => o.status === 'delivering').length === 0 ? (
              <p className="text-sm text-gray-500 text-center py-4">No active deliveries</p>
            ) : (
              deliveryOrders
        ...
        <CardContent>
          <div className="space-y-4">
            {deliveryOrders.filter((o) => o.status === 'ready').length === 0 ? (
              <p className="text-sm text-gray-500 text-center py-4">No orders ready for pickup</p>
            ) : (
              deliveryOrders
                ...
                .map((order) => (
                  <div key={order.id} className="flex items-center justify-between rounded-lg bg-green-50 p-3">
                    ...
                    <Button size="sm" onClick={() => handleOrderAction(String(order.id), 'next')}>
                      Assign Driver
                    </Button>
                  </div>
                ))
...
const AlertsTab = ({ 
  ...
  const hasAlerts = delayedOrders.length > 0 || highPriorityOrders.length > 0;
  ...
  if (!hasAlerts) {
    return (
      <EmptyState
        ...
        description="All orders are running smoothly. Alerts appear here when orders need attention."
        helpText="The system automatically monitors for delayed or high-priority orders."
      />
    );
  }
        ...
        <div className="space-y-3">
          ...
          {delayedOrders.map((order) => (
            <div key={order.id} className="flex items-start gap-3 rounded-lg bg-red-50 p-3">
              ...
              <Button size="sm" variant="outline" onClick={() => handleOrderAction(String(order.id), 'view')}>
                View
              </Button>
            </div>
          ))}
...
export default function OperationsCenter({ orders: initialOrders, stats }: OperationsPageProps) {
      ...
      <Page>
        <div className={`${fullscreen ? 'fixed inset-0 z-50 bg-white' : 'flex h-full flex-col'}`}>
          <Page.Header
            ...
            subtitle="Monitor and manage all active orders in real-time"
            actions={
              <>
                ...
                {/* Only show these controls when there are active orders */}
          ...
          <Page.Content noPadding={fullscreen}>
            {activeOrders.length === 0 ? (
              <EmptyOperationsView fullscreen={fullscreen} />
            ) : (
